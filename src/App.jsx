...
const LoadingSpinner = () => <div className="animate-spin rounded-full h-12 w-12 border-t-2 border-b-2 border-cyan-500"></div>;
...
function MapClickHandler({ setParams }) {
  useMapEvents({
    click(e) {
      ...
      setParams(prevParams => ({ ...prevParams, lat: parseFloat(lat.toFixed(4)), lon: parseFloat(lng.toFixed(4)) }));
    },
  });
  return null;
}
...
function App() {
  ...
  const handleFetchImage = async (e) => {
    e.preventDefault();
    if (!params.apiKey) {
      setError("An API Key (Access Token) is required.");
      return;
    }
    ...
    setTemporalResults([]);
    setInfo(`Submitting job to Copernicus pipeline...`);
  ...
  const ViewerContent = () => {
    ...
    if (error) return <div className="text-red-400 text-center p-4"><p className="font-bold mb-2">Pipeline Error</p><p className="text-sm">{error}</p></div>;
        ...
        <div className="w-full h-full overflow-y-auto p-2 space-y-4">
          {temporalResults.map((result, index) => (
            <div key={index} className="bg-gray-900 rounded-lg p-2">
              <img src={result.image} alt={`Scene from ${result.date}`} className="w-full h-auto object-contain rounded-md" />
    ...
    if (displayedImage) {
      return <img src={displayedImage} alt="Processed satellite view" className="w-full h-full object-contain" />;
    }
        ...
        <main className="grid grid-cols-1 lg:grid-cols-5 gap-8">
          <div className="lg:col-span-2 bg-gray-800 p-6 rounded-lg shadow-2xl h-fit">
            <form onSubmit={handleFetchImage}>
              <div className="mb-4">
                <label htmlFor="apiKey" className="block text-sm font-medium text-gray-300 mb-2">1. Copernicus API Key</label>
                <input type="password" name="apiKey" id="apiKey" value={params.apiKey} onChange={handleParamChange} className="bg-gray-700 w-full p-2.5 rounded-lg" placeholder="Your Access Token" />
              </div>

              <h3 className="text-lg font-semibold mt-6 mb-2 text-gray-200">2. Select Area of Interest</h3>
              <div className="h-64 w-full mb-4 rounded-lg overflow-hidden z-0">
                <MapContainer center={[params.lat, params.lon]} zoom={10} scrollWheelZoom={true} style={{ height: "100%", width: "100%" }}>
              ...
              <div className="grid grid-cols-2 gap-4 mb-4">
                <div>
                  ...
                  <input type="number" step="any" name="lat" id="lat" value={params.lat} onChange={handleParamChange} className="bg-gray-700 w-full p-2.5 rounded-lg" />
                </div>
                <div>
                  <label htmlFor="lon" className="block text-sm font-medium text-gray-300 mb-2">Longitude</label>
                  <input type="number" step="any" name="lon" id="lon" value={params.lon} onChange={handleParamChange} className="bg-gray-700 w-full p-2.5 rounded-lg" />
                </div>
              </div>

              <h3 className="text-lg font-semibold mt-6 mb-2 text-gray-200">3. Time Range & Processing</h3>
              <div className="grid grid-cols-2 gap-4 mb-4">
                <div>
                  <label htmlFor="startDate" className="block text-sm font-medium text-gray-300 mb-2">Start Date</label>
                  <input type="date" name="startDate" id="startDate" value={params.startDate} onChange={handleParamChange} className="bg-gray-700 w-full p-2.5 rounded-lg" />
                </div>
                <div>
                  <label htmlFor="endDate" className="block text-sm font-medium text-gray-300 mb-2">End Date</label>
                  <input type="date" name="endDate" id="endDate" value={params.endDate} onChange={handleParamChange} className="bg-gray-700 w-full p-2.5 rounded-lg" />
                </div>
              </div>
              <div className="mb-6">
                <label htmlFor="scriptType" className="block text-sm font-medium text-gray-300 mb-2">Processing Type</label>
                <select id="scriptType" name="scriptType" value={params.scriptType} onChange={handleParamChange} className="bg-gray-700 w-full p-2.5 rounded-lg">
              ...
              <button type="submit" disabled={isLoading} className="w-full mt-2 flex items-center justify-center bg-cyan-600 hover:bg-cyan-700 disabled:bg-gray-500 font-bold py-3 px-4 rounded-lg transition-transform transform hover:scale-105">
                <PipelineIcon />
          ...
          <div className="lg:col-span-3 bg-gray-800 p-6 rounded-lg shadow-2xl flex flex-col items-center justify-center" style={{height: '720px'}}>
